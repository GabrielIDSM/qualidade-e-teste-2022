<%-- 
    Document   : updatecsv
    Created on : May 31, 2022, 10:44:36 PM
...
<%
String URL = request.getRequestURL().toString().replace("/updatecsv.jsp", "");
...
<html lang="pt-BR">
    <head>
        ...
        <link rel="stylesheet" href="CSS/updatecsv.css">
        <link rel="stylesheet" href="CSS/alert.css">

        <title><%=I18nUtil.i18n("UPDATE_DATA", Language)%></title>
    </head>
    <body class="updatecsv-body">
        ...
        %>
        <div class="updatecsv-content">
            <div class="py-5">
                <%
                    String csvfile = (String) request.getAttribute("csvfile");
                %>
                <div class="container">
                    <h3> <%=I18nUtil.i18n("UPDATE", Language)%> "<%=csvfile%>.csv"</h3>
                    <br/>
                    <form method="POST" action="Csv?a=<%= csvfile %>" enctype="multipart/form-data">
                        <div class="mb-3">
                            <label for="file"><b> <%=I18nUtil.i18n("FILE", Language)%> (<%=csvfile%>.csv) </b></label>
                            <input type="file" class="form-control" name="file" id="file" required>
                        </div>
                        <button type="submit" class="btn btn-outline-primary"> <%=I18nUtil.i18n("SAVE", Language)%> </button>
                        <a class="btn btn-outline-danger" href="Csv?a=list"> <%=I18nUtil.i18n("CANCEL", Language)%> </a>
                    </form>
                </div>
            </div>     
        </div>
